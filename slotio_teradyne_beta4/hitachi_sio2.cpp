...
/* Test09 configs */
#define WAPI_TEST09_ECHO_COUNT 		       1000       /* Number of echos after target temp is reached */
#define WAPI_TEST09_TARGET_TEMP 	       6000       /* Target temp * 100 */
#define WAPI_TEST09_POS_RAMP 	           300        /* Pos ramp * 100 */
#define WAPI_TEST09_NEG_RAMP 	           300        /* Neg ramp * 100 */
#define WAPI_TEST09_SAFE_TEMP 	           3500       /* Safe temp * 100 */
#define WAPI_TEST09_TARGET_TEMP_WAIT_SEC   30*60      /* Max time to reach target temp */
#define WAPI_TEST09_SAFE_TEMP_WAIT_SEC     30*60      /* Max time to reach safe temp */
#define WAPI_TEST09_IDLE_WAIT_SEC          5*60       /* Time spent while at target temp (if WAPI_TEST09_ECHO_COUNT = 0) */
...
static Byte test09(int id, int driveType)
{
    ...
    tv.tv_sec = WAPI_TEST09_TARGET_TEMP_WAIT_SEC;   

    gettimeofday(&tv_tmp,NULL);
    timeradd(&tv,&tv_tmp,&tv_end);

	/* Ramp up to target temp */
	while(1)
	{
        printTemperatureVariables(id);
		rc = isOnTemp(id);
    	...
    	if ( timercmp(&tv, &tv_end, >) ) {
			terprintf("ERROR: Did not reach target temp on time \n");
			rc = 1; goto test09_exit;
		}
	
	}

	/* Do some echos or wait at the target temperature */
	if(loopcount) {
	...
	/* Go to safe handling temp */
	rc = setSafeHandlingTemperature(id, WAPI_TEST09_SAFE_TEMP);

    memset(&tv,0,sizeof(struct timeval));
    tv.tv_sec = WAPI_TEST09_SAFE_TEMP_WAIT_SEC;   
	...
	while(1)
	{
        printTemperatureVariables(id);
		rc = getCurrentTemperature(id, &currentTemp);
		if(currentTemp <= WAPI_TEST09_SAFE_TEMP) {
			break;
		}
		sleep(5);
		...
		gettimeofday(&tv,NULL);
    	if ( timercmp(&tv, &tv_end, >) ) {
			terprintf("ERROR: Did not reach safe handling temp on time \n");
			rc = 1; goto test09_exit;
		}
	}
