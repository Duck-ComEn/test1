...
#define WAPI_PRINT_JOURNAL_ALL 			0
#define WAPI_PRINT_JOURNAL_ON_ERROR		100
...
TER_HtempInfo siHtempInfo;		/* This global variable is used for wapitest */
...
static void printJournal(Byte bCellNo, Word wCount);
...
#if WAPI_ROW_BASED_TEMP_ADJ_ENABLE
static int  terReadTempAdjustments(Byte bCellNo);
static int  siTempAdjustmentOffset = 0;
#endif
/*
...
static int  siLogLevel = WAPI_DEFAULT_LOG_LEVEL;
static char logFileName[128] = {'\0'};
static Word siTemperatureTarget = 0;
static int  siReadHtempConfig = 0;
...
Byte clearFPGABuffers(Byte bCellNo, int bufferId) {
	...
	default:
		...
		TLOG_ERROR("ERROR %d : invalid reset value was passed",bufferId);

		// return error
		return TerError;
		
	}
	WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_ResetSlot(wapi2sio(bCellNo), resetType));
...
Byte siInitialize(Byte bCellNo)
{
	...
	if( 0 == terGetWapiTestMode(bCellNo) ) {  /* Certain wapitest command line will not work if slot reset is called and hence this code */
		...
		if ( rc ){
			TLOG_ERROR("ERROR: isCellEnvironmentError returned error 0x%x \n", rc);
			return TerError;
		}
	}
	else {
    ...
    if ( TER_Status_none != status ) {
    	...
    	TLOG_TRACE("DioFirmwareVersion: <DIO App:%s> <924x Init:%d.%d> <924x App:%d.%d>\n", pSlotInfo.DioFirmwareVersion, 
		            dp.dp_m9240_major, dp.dp_m9240_minor, dp.dp_k9240_major, dp.dp_k9240_minor);
    	TLOG_TRACE("DioBoardType: <%x>\n", dp.dp_dio_board_type);
    	...
    	TLOG_TRACE("SioTeradynePartNum: <%s>\n", pSlotInfo.SioPcbPartNum);
    	TLOG_TRACE("DioTeradynePartNum: <%s>\n", pSlotInfo.SlotPcbPartNum);
		
	    rc = ns2->TER_GetNeptuneSio2BuildVersion ( &build_version );
        if ( TER_Status_none != status ) {
		   terError(__FUNCTION__, bCellNo, ns2, status);
		   return TerError;
        } 
		TLOG_TRACE("Neptune SIO2 Lib (neptune_sio2.a): %d\n", build_version);
		TLOG_TRACE("SIO2 Super App (SIO2_SuperApplication): %s\n", pSlotInfo.SioAppVersion);
	...
	if(str64kMaxPendingPerFPGA != NULL) {
		if( (isdigit(str64kMaxPendingPerFPGA[0])) && 
		    (isdigit(str64kMaxPendingPerFPGA[1]) || str64kMaxPendingPerFPGA[1] == '\0') ) {
	    	si64kMaxPendingPerFPGA = atoi(str64kMaxPendingPerFPGA);
			si64kMaxPendingPerFPGA = si64kMaxPendingPerFPGA > 35 ? 35:si64kMaxPendingPerFPGA;
		}
	}


#if WAPI_ROW_BASED_TEMP_ADJ_ENABLE
	/* read temp adjustments from csv file */
	siTempAdjustmentOffset = 100*terReadTempAdjustments(bCellNo); 
    TLOG_TRACE("Info : Temperature adjustment offset for this slot = %d \n", siTempAdjustmentOffset); 
#endif 

	siReadHtempConfig = 1;  /* This global signals htemp alg to read config from .csv file */
...
Byte siFinalize(Byte bCellNo)
{
	...
	if( siLogLevel == 4) {
		printJournal(bCellNo, WAPI_PRINT_JOURNAL_ALL);
	}
...
Byte isCellEnvironmentError(Byte bCellNo)
{
	...
	if (status != TER_Status_none) {
		...
		if(tccb.optionDevelopmentStation){
	          TLOG_ENTRY("option temp error will be ignore : slot_errors = 0x%0x\n",slotStatus.slot_errors);
		  slotStatus.slot_errors = slotStatus.slot_errors & ~TER_Notifier_TempRampNotCompleted & ~TER_Notifier_TempEnvelopeFault;
                }
		if (slotStatus.slot_errors) {
			rc = TerError;
		    TLOG_ERROR("ERROR : slot_errors = 0x%0x\n", slotStatus.slot_errors);
		}
	}

    TLOG_EXIT("Exit : %s, rc=%s 0x%0x\n", __FUNCTION__, rc ? "Yes":"No", slotStatus.slot_errors);
	return rc;
}
	...
	if (status != TER_Status_none) {
		...
		*wTempInHundredth = (Word)(slotStatus.temp_drive_x10 * 10);
        ...
        if ( *wTempInHundredth >= wTempInHundredthMaxLimit){
	        TLOG_ERROR("ERROR: %s failed because slot temperature over %d degreeC \n",__FUNCTION__,wTempInHundredthMaxLimit/100);
		return TerError;
        }
...
#if WAPI_ROW_BASED_TEMP_ADJ_ENABLE
    if( wTempInHundredth > WAPI_TEMP_ADJ_TARGET_TEMP_MIN ) {  /* Adjust temp if target temp is > 45C */
		wTempInHundredth += siTempAdjustmentOffset;
		TLOG_TRACE("Info : Adjusted target temperature = %d \n", wTempInHundredth);
	}
#endif
...
Byte getTemperatureErrorStatus(Byte bCellNo, Word *wErrorStatus)
{
	...
	*wErrorStatus = 0;
	...
	if (slotStatus.slot_errors) {
		...
		TLOG_ERROR("ERROR : %s, rc=%d, 0x%x, 0x%x\n", __FUNCTION__, rc, *wErrorStatus, slotStatus.slot_errors);
	}
...
Byte getCurrentVoltage(Byte bCellNo,Word *wV5InMilliVolts, Word *wV12InMilliVolts)
{
	...
	if (status != TER_Status_none) {
		...
		*wV12InMilliVolts = (Word)(slotStatus.voltage_actual_12v_mv);
        ...
        if(tccb.optionDevelopmentStation){ 
	  TLOG_ENTRY("option temp error will be ignore");
...
Byte setVoltageRiseTime(Byte bCellNo, Word wV5TimeInMsec, Word wV12TimeInMsec)
{
	Byte rc = Success; /* No error */
	TER_Status status;
	int t5VRiseTimeInMsec, t5VFallTimeInMsec, t12VRiseTimeInMsec, t12VFallTimeInMsec;
	...
	if(wV12TimeInMsec) {  /* No need to set rise time if zero */
    	...
    	WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_Get12VRiseAndFallTimes ( wapi2sio(bCellNo), &t12VRiseTimeInMsec, &t12VFallTimeInMsec ));
    	if ( TER_Status_none != status ) {
			terError(__FUNCTION__, bCellNo, ns2, status);
			return TerError;
    	}
    	...
    	WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_Set12VRiseAndFallTimes ( wapi2sio(bCellNo), (int)wV12TimeInMsec, t12VFallTimeInMsec ));
    	if ( TER_Status_none != status ) {
			terError(__FUNCTION__, bCellNo, ns2, status);
			return TerError;
		}
...
Byte getVoltageRiseTime(Byte bCellNo, Word *wV5TimeInMsec, Word *wV12TimeInMsec)
{
	...
	*wV12TimeInMsec = (Word)t12VRiseTimeInMsec;
...
Byte setVoltageFallTime(Byte bCellNo, Word wV5TimeInMsec, Word wV12TimeInMsec)
{
	...
	if(wV12TimeInMsec) {  /* No need to set rise time if zero */
    	...
    	/* Set 12V rise/fall time */
    	WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_Set12VRiseAndFallTimes ( wapi2sio(bCellNo), t12VRiseTimeInMsec, (int)wV12TimeInMsec));
    	if ( TER_Status_none != status ) {
			terError(__FUNCTION__, bCellNo, ns2, status);
			return TerError;
		}
...
Byte getVoltageFallTime(Byte bCellNo, Word *wV5TimeInMsec, Word *wV12TimeInMsec)
{
	...
	*wV12TimeInMsec = (Word)t12VFallTimeInMsec;
...
Byte setVoltageInterval(Byte bCellNo, int wTimeFromV5ToV12InMsec)
{
	...
	if( wTimeFromV5ToV12InMsec > 127 || wTimeFromV5ToV12InMsec < -127 ) {
		TLOG_ENTRY("ERROR: Invalid interval value %d. Valid range -127 to +127", wTimeFromV5ToV12InMsec);
		return TerError; 
	}
	
  
    /* Set 5V & 12V sequencing time */
    WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_Set5V12VSequenceTime ( wapi2sio(bCellNo), wTimeFromV5ToV12InMsec ));
	if (status != TER_Status_none) {
		terError(__FUNCTION__, bCellNo, ns2, status);
		return TerError;
	}
...
Byte getVoltageErrorStatus(Byte bCellNo, Word *wErrorStatus)
{
	...
	*wErrorStatus = 0;
    ...
    TLOG_EXIT("Exit : %s, rc=%d, 0x%x, 0x%x\n", __FUNCTION__, rc, *wErrorStatus, slotStatus.slot_errors);
	return rc;
}
...
/**
   ...
   * </pre>
   *****************************************************************************/
Byte siReadDriveMemory2KAndLess(Byte bCellNo, Dword dwAddress, Word wSize, Byte *bData, Word wTimeoutInMillSec)
{
    Byte 			*recvBuf = NULL;
...
/**
   ...
   * </pre>
   *****************************************************************************/
Byte siReadDriveMemoryMulti(Byte bCellNo, Dword dwAddress, Word wSize, Byte *bData, Word wTimeoutInMillSec)
{
    Byte 	rc = 0;
	Word  	size, totalSize;
	Byte    *ptrData;
	...
	while(totalSize > 0) {
		...
		rc = siReadDriveMemory2KAndLess(bCellNo, rAddr, size, ptrData, wTimeoutInMillSec);
		
		if(rc == 0) {
   			TLOG_TRACE("Slot %d: siReadDriveMemory, Read %d bytes from Address 0x%08x \n", bCellNo, size, (int)rAddr);
			rAddr += (size/2);
			ptrData += size;
		}
		else {
	    	TLOG_TRACE("Slot %d: siReadDriveMemory,failed \n", bCellNo);
			break;
 		}
	}
	return rc;
...
Byte siReadDriveMemory(Byte bCellNo, Dword dwAddress, Word wSize, Byte *bData, Word wTimeoutInMillSec)
{
	...
	if(wSize < WAPI_MAX_BYTES_FROM_SMALL_BUFF) {
		WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_SioSendBuffer(wapi2sio(bCellNo), cmdLength, sendBuf));
		if (status != TER_Status_none) {
			terError(__FUNCTION__, bCellNo, ns2, status);
			return TerError; /* Some error exists */
		...
		recvBuf = (Byte *)malloc(WAPI_SEND_RECV_BIG_BUF_SIZE);
 	    ...
 	    WAPI_CALL_RPC_WITh_RETRY(status = ns2->TER_SioHugeBufferWriteRead (wapi2sio(bCellNo), cmdLength, sendBuf,
					  (int *)&respLength, recvBuf, wTimeoutInMillSec/1000, &status1, si64kMaxPendingPerFPGA));
		
		/* If too many requests are pending the queue, break and optimize */
		if(status == TER_Status_64k_count_exceeded) {
			TLOG_TRACE("Info : Slot %d TER_Status_64k_count_exceeded. Too many requests for 64K FIFO. Breaking this request for optimization \n", bCellNo);
			rc = siReadDriveMemoryMulti(bCellNo, dwAddress, wSize, recvBuf, wTimeoutInMillSec);
			free(recvBuf);
			return rc;
		}
	}
...
static void printJournal(Byte bCellNo, Word wCount)
{
    ...
    if ( TER_Status_none != status ) {
		...
		if( wCount == WAPI_PRINT_JOURNAL_ALL ) {
			maxLines = jb->cnt;
		}
		else {			
			maxLines = (jb->cnt > wCount) ? wCount : jb->cnt;
		}
...
void terPrintSlotInfo(Byte bCellNo)
{
    ...
    if ( TER_Status_none != status ) {
    	...
    	status = ns2->TER_IsDioPresent ( wapi2sio(bCellNo), &dp );
    	if ( status != TER_Status_none ) {
			printf("ERROR: terPrintSlotInfo failed. TER_IsDioPresent() failed with status = %d. \n", status);
		}

    	printf("DioFirmwareVersion: <DIO App:%s> <924x Init:%d.%d> <924x App:%d.%d>\n", pSlotInfo.DioFirmwareVersion, 
		            dp.dp_m9240_major, dp.dp_m9240_minor, dp.dp_k9240_major, dp.dp_k9240_minor);
    	printf("DioBoardType: <%x>\n", dp.dp_dio_board_type);
	    ...
	    status = ns2->TER_GetNeptuneSio2BuildVersion ( &build_version );
    	if ( TER_Status_none != status ) {
			printf("ERROR: terPrintSlotInfo:  TER_GetNeptuneSio2BuildVersion() failed with status = %d. \n", status);
    	} 
...
static void terError(const char *funcName, Byte bCellNo, Neptune_sio2 *ns2, TER_Status status)
{
	...
	/* Log SIO journal if enabled */
	if( siLogLevel >= 3) {
		printJournal(bCellNo, WAPI_PRINT_JOURNAL_ON_ERROR);
	}
...
{
	TER_Status status;
    unsigned int respLength = 10; 

	status = ns2->TER_SioReceiveBuffer(wapi2sio(bCellNo), &respLength, 0, sendRecvBuf, status1); 
	if (status != TER_Status_none) {
		terError(__FUNCTION__, bCellNo, ns2, status);
	}
...
#if WAPI_ROW_BASED_TEMP_ADJ_ENABLE
/*!
 * read temp offsets from csv file
 */
static int terReadTempAdjustments(Byte bCellNo)  
{ 
    FILE *fp = NULL;  
	...
	fp = fopen("/var/tftp/TempAdjustments.csv", "r");
	if (fp == NULL) {
		TLOG_ERROR("ERROR: Unable to open /var/tftp/TempAdjustments.csv \n");
		return(retVal);
	}
    ...
    {  
		for(i = 1; i <= bCellNo; ++i) {
			token = ((i == 1) ? strtok(Line, ", ") : strtok(NULL, ", "));
			if( token == NULL ) {
				break;
			}
		}
	...
	/* If a token is found, save for temp compensation */
	if( token != NULL ) {
 		retVal = atoi(token);
	} 
	else {
		TLOG_ERROR("ERROR: temp compensation token not found in /var/tftp/TempAdjustments.csv \n");
	}
...
#define 	WAPI_HTEMP_ADJ_ENABLED 			1
#define 	WAPI_HTEMP_DELTA_MIN 			-200     /* delta temp * 100 */
#define 	WAPI_HTEMP_DELTA_MAX 			200      /* delta temp * 100 */
#define 	WAPI_HTEMP_ADJ_INCREMENTS		200
#define 	WAPI_HTEMP_ADJ_MAX_SET_POINT	8000     /* max setpoint * 100 */
#define  	WAPI_HTEMP_WAIT_TIME_OFFSET_SEC	120
...
Byte adjustTemperatureControlByDriveTemperature(Byte bCellNo, Word wTempInHundredth)
{
	Byte rc = Success; /* No error */
	TER_Status status;
	static TER_HtempInfo slot;
	Word  currentTemperature, posRampRate;

    FILE *fp = NULL;  
    char Line[1000];  
    char *token = NULL;  
	int  i, config[6];  /* six configurables */


	TLOG_ENTRY("Entry: %s(%d, %d) \n", __FUNCTION__, bCellNo, wTempInHundredth);
	
	if( siReadHtempConfig) {  /* this flag is set in siInitialize */
		fp = fopen("/var/tftp/htemp_config.csv", "r");
		if (fp == NULL) {
			TLOG_ERROR("ERROR: Unable to open /var/tftp/htemp_config.csv \n");
		}
		else {
			fgets(Line, sizeof(Line), fp); /* first line is comment. Read and ignore */
    		while(fgets(Line, sizeof(Line), fp))  
    		{  
				for(i = 0; i <= 5; ++i) {
					token = ((i == 0) ? strtok(Line, ", ") : strtok(NULL, ", "));
					if( token == NULL ) {
						break;
					}
					config[i] = atoi(token);
		...
		/* If a token is not found, use defaults */
		if( token == NULL || fp == NULL) {
 			TLOG_ERROR("ERROR: problem with /var/tftp/htemp_config.csv. Using defaults \n");
			slot.hTempAdjustmentEnabled = WAPI_HTEMP_ADJ_ENABLED;
			slot.hTempAdjustmentWaitTimeOffset = WAPI_HTEMP_WAIT_TIME_OFFSET_SEC;
			slot.hTempDeltaMin = WAPI_HTEMP_DELTA_MIN;
			slot.hTempDeltaMax = WAPI_HTEMP_DELTA_MAX;
			slot.hTempAdjustmentIncrementSize = WAPI_HTEMP_ADJ_INCREMENTS;
			slot.hTempAdjustmentMaxSetPoint = WAPI_HTEMP_ADJ_MAX_SET_POINT;

		}
		else {
			/*	Sample csv file: /var/tftp/htemp_config.csv
			hTempAdjustmentEnabled, hTempAdjustmentWaitTimeOffset, hTempDeltaMin, hTempDeltaMax, hTempAdjustmentIncrementSize, hTempAdjustmentMaxSetPoint
			1,                      600,                             3,             4,             5,                            6, 	*/

			slot.hTempAdjustmentEnabled = config[0];
			slot.hTempAdjustmentWaitTimeOffset = config[1];
			slot.hTempDeltaMin = config[2];
			slot.hTempDeltaMax = config[3];
			slot.hTempAdjustmentIncrementSize = config[4];
			slot.hTempAdjustmentMaxSetPoint = config[5];
			TLOG_TRACE("INFO: Enable %d, WaitOffset %d, DeltaMin %d, DeltaMax %d, IncSize %d, MaxSetPoint %d \n", slot.hTempAdjustmentEnabled,slot.hTempAdjustmentWaitTimeOffset,
			slot.hTempDeltaMin, slot.hTempDeltaMax, slot.hTempAdjustmentIncrementSize, slot.hTempAdjustmentMaxSetPoint);
		}
	}
 
 	/* Do not make htemp correction if drive temp is not within a valid range (10C - 100C) */
	if (slot.hTempAdjustmentEnabled && (wTempInHundredth > 1000) && (wTempInHundredth < 10000) ) {
	
		slot.htemp = wTempInHundredth;
		slot.targetTemperatureInitial = siTemperatureTarget; /* unadjusted initial target temp */
		slot.targetTemperatureReached = isOnTemp(bCellNo);

		if (slot.targetTemperatureReached) 	{
			/* get current sled temp */
			rc = getCurrentTemperature(bCellNo, &currentTemperature);
			if(rc) {
				TLOG_ERROR("ERROR: getCurrentTemperature failed \n");
				rc = TerError; goto adjustTemperatureControlByDriveTemperature_exit;
			}
			else {
				slot.currentTemperature = (int)currentTemperature;  
			}

			/* get ramp rates. Assuming symmetrical ramp rates */
			rc = getPositiveTemperatureRampRate(bCellNo, &posRampRate);
			if(rc) {
				TLOG_ERROR("ERROR: getPositiveTemperatureRampRate failed \n");
				rc = TerError; goto adjustTemperatureControlByDriveTemperature_exit;
			}
			else {
				slot.positiveRampRate = posRampRate;	
			}

    		status = ns2->TER_SetHitachiTemp(wapi2sio(bCellNo), &slot);
    		if ( TER_Status_none != status ) {
				terError(__FUNCTION__, bCellNo, ns2, status);
				rc = TerError; goto adjustTemperatureControlByDriveTemperature_exit;
    		}
			
			memcpy(&siHtempInfo, &slot, sizeof(slot)); /* Need this for wapitest */
			
			TLOG_TRACE("INFO: on_t %d, ramp %d, cur_t %d tgt_t %d, tgt_i %d, h_t %d, delta_t %d, adj_tm %d, wait_tm %d, delta_tm %d, delta_in %d \n", 
...
adjustTemperatureControlByDriveTemperature_exit:
	return rc;
}
