...
Byte siInitialize(Byte bCellNo)
{
	...
	if( rc != TER_Status_none ) {
		if (rc == TER_Status_no_client_connection) {
...
Byte siSetDebugLogLevel(Byte bCellNo, Byte bLogLevel)
{
	...
	sprintf(logFileName, "slot%03d_%s.txt", bCellNo, strDateTime);
...
Byte adjustTemperatureControlByDriveTemperature(Byte bCellNo, Word wTempInHundredth)
{
    ...
    status = ns2->TER_SetHitachiTemp(wapi2sio(bCellNo), wTempInHundredth);
    if ( TER_Status_none != status ) 
	{
		terError(__FUNCTION__, bCellNo, ns2, status);
		rc = TerError;
