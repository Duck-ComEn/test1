...
#define WRAPPER_IMPLEMENTATION_VERSION "20100615a-ChrisB"
	// setUartPullupVoltage support for XCALIBRE and OPTIMUS (each requires updated firmware)
	// handle CCCTypes for Optimus (ObtainAndLogCellFirmwareLimitsAndVersions in envfuncs.cpp)
	
//#define WRAPPER_IMPLEMENTATION_VERSION "20100604a-ChrisB"
	// siSetDriveFanRPM
	...
	if ( pCellEnv == NULL)
	{
		...
		if ( pCellEnv != NULL)
		{
			...
			{	
				...
				pCellEnv->SetLEDA( lsOn );  // [DEBUG] turn on LEDs to notify everyone working near CELLs which I run testcase
				pCellEnv->SetLEDB( lsOn );  // [DEBUG] turn on LEDs to notify everyone working near CELLs which I run testcase
...
//----------------------------------------------------------------------
{
	...
	if (pCellEnv != NULL) 
	{
		...
		setSafeHandlingTemperature(bCellNo, 3500);
    
		pCellEnv->SetLEDA( lsOff );  // [DEBUG] //(we turned on in Initialize)
		pCellEnv->SetLEDB( lsOff );  // [DEBUG] //(we turned on in Initialize)
...
//----------------------------------------------------------------------
{
	...
	*wV12InMilliVolts = 0;
	...
	uint16 v = 0;
		...
		if (xyrc == xyrcOK)
		{
			//LOG_INFO "%s,%d: getTargetVoltage( ) GetAllSupplyOnOffStates() returned sidmask of 0x%x\n", __func__, bCellNo, sidmask );
			if ((sidmask & psda[bCellNo].PSU_sid_5V) == 0)
			{
				*wV5InMilliVolts = 0;
			}	
