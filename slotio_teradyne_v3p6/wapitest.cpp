...
DRIVE_INFO driveInfo[3] = { {10,	540,    3,     	0,      0x08331000, 0x8331063,  0,		2700,				1843200},	   	/* EC7 	*/
						    {10,	540,    3,     	0,      0x08331000, 0x8331064,  0,		2700,				1843200},		/* EB7	*/
...
static void showApiHelp() {
	...
	printf("   snm <SLT>                             - siGetNotifierMask\n");
	printf("   gnm <SLT>                             - siSetNotifierMask\n");
	printf("---------- Notes ----------\n");
...
//		Data[0] = i; Data[1] = i;
		...
		rc = siWriteDriveMemory(id, wAddr, size, Data, WAPI_DRIVE_TIMEOUT_MS);
    	if(rc != 0) {
	    	++errorCnt;
...
/**
   ...
   * </pre>
   *****************************************************************************/
static Byte test11(int id, int driveType)
{
    Byte 	rc = 0;
	int 	j, i, errorCnt = 0;
	Dword   size = 1024, rAddr;
Byte Data [1024];
	...
	rc = siReadDriveMemory(id, rAddr, size, Data, WAPI_DRIVE_TIMEOUT_MS);
	if(rc != 0) {
		printf ("Error reading drive memory: %d\n", rc);
		++errorCnt;
	}
	if ( (rc = siEchoDrive(id, 512, Data, WAPI_DRIVE_TIMEOUT_MS)) !=0 ) {
		printf ("Error during 1st echo: %d\n", rc);
		++errorCnt;
	}
	...
	for (j = 0; j< 5; j++) {
		...
		rc = setTargetVoltage(id, 0, 0);
		printf("Power is off.  Waiting 2 seconds to turn it back on...\n");
		sleep (2); printf("Turning power back on.\n");
		rc = setTargetVoltage(id, 5000, 12000);
		printf ("Power is on, waiting 10 seconds to clear receive buffer...\n");
		sleep (10);
		printf("Clearing read buffer.\n");
		clearFPGABuffers (id, fpgaBufferTypeReceive);
		printf("Starting echo tests.\n");

		for(i = 0; i < 3; ++i)
		{
			if ( (rc = siEchoDrive(id, 512, Data, WAPI_DRIVE_TIMEOUT_MS)) != 0) {
				printf ("Error during %d,%dth echo: %d\n", j, i, rc);
				++errorCnt;
			}
		}
	}
...
/**
   ...
   * </pre>
   *****************************************************************************/
static Byte test12(int id, int driveType)
{
	Byte rc = 0;
	TER_Notifier rmask ;
	unsigned int mask ;

	printf("Enter Error to mask -> ");
	scanf("%x", &mask);
	rc = siSetNotifierMask(id, (TER_Notifier)mask, 1);
	if(rc == 0)
		printf("siSetNotifierMask: Success, Set to 0x%x \n", (unsigned int)mask);
	else
		printf("siSetNotifierMask: Failed \n" );
	printf ("\nReading mask back\n");
	rc = siGetNotifierMask(id, &rmask);
	if(rc == 0)
		printf("siGetNotifierMask: Success, mask is 0x%x \n", (unsigned int)rmask);
	else
		printf("siGetNotifierMask: Failed \n" );
	test04(id, driveType);

	// Clear the mask and run the test again
	rc = siSetNotifierMask(id, (TER_Notifier)mask, 0);
	if(rc == 0)
		printf("siSetNotifierMask: value x%x, disabled \n", (unsigned int)mask);
	else
		printf("siSetNotifierMask: Failed \n" );
	printf ("\nReading mask back\n");
	rc = siGetNotifierMask(id, &rmask);
...
static Byte testApi(char *cmd, Byte id, int *p)
{
    ...
    else if (ISCMD('g','e','c'))
    {
    	...
    	rc = siSetNotifierMask(id, (TER_Notifier)mask, 1);
    	if(rc == 0)
    		printf("siGetNotifierMask: Success, Set to 0x%x \n", (unsigned int)mask);
    	else
    		printf("siGetNotifierMask: Failed \n" );
    	printf ("\nReadying mask back\n");
    	rc = siGetNotifierMask(id, &rmask);
	...
	else if (ISCMD('t','1','1')) {	 /* Test echo after power off/on */
		rc = test11(id,driveType);
	}
	else if (ISCMD('t','1','2')) {	 /* Test slot errors masking */
		rc = test12(id,driveType);
	}
