...
Common::Common(void) {
	...
	sioSlotErrorsMask = TER_Notifier_Cleared;
}
...
/*
 * Name			:	SetSlotErrorsMask
 * Description	:	Mask certain Notifier errors
 * Input		:	Notifier mask
 * Return		:	0 = success
 * 					Non-Zero = Failure
 */
Byte Common::SetSlotErrorsMask(TER_Notifier mask, int enable) {
	Byte rc = success;

	// The only bit allowed to be masked currently is the TER_Notifier_TempRampNotCompleted
	if (mask != TER_Notifier_TempRampNotCompleted)
		rc = invalidArgumentError;

	if (enable != 0)
		sioSlotErrorsMask |= mask;
	else
		sioSlotErrorsMask &= !mask;
...
/*
 * Name			:	GetSlotErrorsMask
 * Description	:	Get the values that will mask the Notifier flag
 * Input		:	None
 * Return		:	Current masked errors
 *
 */
TER_Notifier Common::GetSlotErrorsMask(void) {

	return (TER_Notifier)sioSlotErrorsMask;
}

/*
 * Name			:	maskRequestedErrors
 * Description	:	mask slot errors per the Notifier mask
 * Input		:	unmasked slot_errors
 * Output		:	masked slot_errors
 * Return		:	void
 *
 */
void Common::maskRequestedErrors (TER_Notifier *slot_errors) {
	unsigned int slotErrors, errorsMask = GetSlotErrorsMask();

	if (errorsMask && *slot_errors) {
		WriteToLog(LogTypeError, "Error Mask: = 0x%0x\n", errorsMask);
		WriteToLog(LogTypeError, "slot_errors: = 0x%0x\n", *slot_errors);
	}
	slotErrors = *slot_errors & ~errorsMask;
	*slot_errors = (TER_Notifier)slotErrors;
}
/*
 * Name			:	SetLogLevel
...
Byte Common::ValidateUart2ReceivedData(Byte *sendRecvBuf, Word cmdLength,
		...
		TER_Status status, unsigned int additionalFault) {
	...
	if (status == TER_Status_transmitter_is_halted) {
		WriteToLog(LogTypeError, "ERROR: Transmit halted in FPGA due to error \n");

		// Find out why transmit is halted
		Byte error = driveAckTimeout;

		// Check if there was a bad ack from the drive otherwise
		// it timed out
		for (i = 0; i < respLength; i++) {
			if (sendRecvBuf[i] != success &&
					sendRecvBuf[i] != uart2AckSuccess) {
				WriteToLog(
						LogTypeError,
		...
		RET(error);
	} else if (status != TER_Status_none){
		LogError(__FUNCTION__, slotIndex, status);
		RET(GetWrapperErrorCode(status, TER_Notifier_Cleared));
	}
