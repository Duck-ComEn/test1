...
//	FILE* logFile;
	...
	unsigned int sioSlotErrorsMask;
...
public:
	...
	Byte ValidateUart2ReceivedData(Byte *sendRecvBuf, Word cmdLength, unsigned int respLength,
	                                 unsigned int expLength, Word *pBuffer, TER_Status status,
	                                 unsigned int additionalFault);


	/*
	 * UART3 Support methods
	 */
//	Byte Uart3LinkControl(Byte bCellNo, Byte bReqRespOpcode, Byte bFlagByte, Word *wPtrDataLength, Word wDataSequence, Byte *DataBuffer, Dword Timeout_mS);
	Byte Uart3GetParametersLink(Byte bCellNo, Byte bReqRespOpcode, Byte bFlagByte, Word *wPtrDataLength, Word wDataSequence, Byte *DataBuffer, Dword Timeout_mS);
	...
	Byte SetSlotErrorsMask(TER_Notifier mask, int enable);
	TER_Notifier GetSlotErrorsMask(void);
	void maskRequestedErrors (TER_Notifier *slot_errors);
