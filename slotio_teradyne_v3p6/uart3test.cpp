...
//	Dword uart3BaudRate = 8333000;
	...
	CCB_DEBUG ccb_obj_prototype_startstopunit = { 6, 0, 0, { 0x1B, 0x00, 0x00,
			0x00, 0x00, 0x00 }, { 0 }, { 0 }, 0, 0, 0, 0, 480 * 1000, 0 };
	...
	CCB_DEBUG ccb_obj_prototype_readbuffer = { 10, 0, 0, { 0x3C, 0x02, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, { 0 }, { 0 }, 0, 0, 0,
			0, 480 * 1000, 0 };
	CCB_DEBUG ccb_obj_prototype = { 10, 0, 0, { 0 }, { 0 }, { 0 }, 0, 0, 0, 0, 480
			* 1000, 0 };
	CCB_DEBUG ccb_obj;
...
*/
	...
	if (rc == uart3AckDataAvailable_DriveRetainsControl) {
		...
		printf("siGetUARTParametersRequest to get current DMA timeout Failed with error code 0x%x\n", rc);
	}
	...
	if (rc == uart3AckReady) {
		printf("siSetUARTParametersRequest to change DMA timeout Passed\n");
		...
		if (rc == uart3AckDataAvailable_DriveRetainsControl) {
			...
			printf("siGetUARTParametersRequest to verify DMA timeout Failed with error code 0x%x\n", rc);
		}
	/*  Failed setting new DMA */
	} else {
		printf("siSetUARTParametersRequest to change DMA timeout Failed with error code 0x%x  Retrying... \n", rc);
		sleep (2);
		rc = siSetUARTParametersRequest(bCellNo, &parameterData, dwTimeout);
		if (rc != uart3AckReady) {
			printf("siSetUARTParametersRequest to change DMA timeout Failed with error code 0x%x  Retries exhausted... \n", rc);
			return rc;
		}
	}
	...
	for (Dword i = 0; i < dwMainLoopCount; i++) {
		...
		for (unsigned long ii=0;;ii++) {
			...
			rc = execCCBCommand(&dcb, bufDownload);
			if ( (rc == respPacketLengthMismatch) || (rc == checksumMismatchError) || (rc == invalidDataLengthError) ) {
			...
			//  Verify the data we just wrote
				...
				if ( (rc != uart3AckSenseAvailable) && (rc != uart3AckQueueFull) ) {
					...
					if ((rrc = execCCBCommand(&ucb, bufCompare)) != uart3AckSCSICommandComplete) {
						printf ("Unable to verify write.  Read compare rc is 0x%x\n",rrc);getchar();
					}
					else {
	...
	elapsedTime = Common::Instance.GetElapsedTimeInMillSec(start, end);
	...
	gettimeofday(&start, NULL);
		...
		printf("Read Start\n");
		for (unsigned long ii=0;;ii++) {
			memset(bufDownload, 0x0, MAX_DATA_BUFFER_LENGTH);
			rc = execCCBCommand(&ucb, bufDownload);
			if ( (rc == respPacketLengthMismatch) || (rc == checksumMismatchError) || (rc == invalidDataLengthError) ) {
		        printf ("rc from read execCCBCommand = 0x%x !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", rc);
				printf ("during %ldth of %ld loops, Press ENTER to continue\n",
						ii, dwDownloadSize/MAX_DATA_BUFFER_LENGTH);
            }
	...
	// Back to Uart2 mode
	// ========================================
	if ((rc = siSetUart2ModeRequest(bCellNo, 30000)) != uart2AckSuccess) {
	                printf("Error: resetting Uart2 Mode 0x%x\n", rc);
	                return rc;
	}
    if ((rc = siSetUartBaudrate(bCellNo, uartBaudRate)) != success) {
	                printf("Error: while setting baudrate 0x%x\n", rc);
	                return rc;
    }
    else
...
Byte uart2Echo(Byte slotIndex)
{
        // =======================
                Byte Data[2048];
                Byte status = siEchoDrive(slotIndex, 512, Data, 500);
            if(status == success ) {
                int i;
                for (i = 0; i < (int)512; i++) {
                                if(isprint(Data[i]))
